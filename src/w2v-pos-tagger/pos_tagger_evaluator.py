...
def run_evaluation_for(corpus, framework, analyse=False):
    ...
    tprint(results)
    results.to_csv(path.join(OUT_DIR, '{}_{}_{}_results.csv'.format(corpus, framework, TAGSET)), sep='\t')
...
def tag_evaluator_main():
    """evaluate the tagging with several metrics. Each corpus for each framework.
...
if __name__ == '__main__':
    # load original tiger corpus
    TO_DF = get_preprocessed_corpus(TIGER)
    # load original hdt corpus
    HO_DF = get_preprocessed_corpus(HDT)

    # Evaluate for both STTS and Universal tagsets by default.
    # Evaluation can be restricted to only one tagset by given --STTS or --UNIV as argument.
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] == '--STTS'):
        TAGSET = STTS
        TAGSET_TAGS = STTS_TAGS
        tag_evaluator_main()
    if len(sys.argv) == 1 or (len(sys.argv) == 2 and sys.argv[1] == '--UNIV'):
        TAGSET = UNIV
