...
KEYS = {
    TIGER: [TOKN_ID, FORM, LEMM, 'PLEMMA', STTS, 'PPOS', 'FEAT', 'PFEAT', 'HEAD',
...
# corpora file names
FILES = {
    TIGER: [path.join(DATA_DIR, 'tiger_release_aug07.corrected.16012013.conll09')],
    HDT: [path.join(DATA_DIR, f) for f in ['part_A.conll', 'part_B.conll', 'part_C.conll']],
    PREPROCESSED: lambda corpus: path.join(DATA_DIR, '{}_preprocessed.csv'.format(corpus)),
    SELFTAGGED: lambda corpus, framework: path.join(DATA_DIR, '{}_pos_by_{}.csv'.format(corpus, framework))
}

# tags from STTS website.
# Replaced PAV with PROAV since all corpora do so as well
# mapping according to de-tiger.map
...
# removed NNE from mapping since no official STTS tag
STTS_UNI_MAP = {
    '$(': '.', '$,': '.', '$.': '.', 'ADJA': 'ADJ', 'ADJD': 'ADJ', 'ADV': 'ADV', 'APPO': 'ADP',
    'APPR': 'ADP', 'APPRART': 'ADP', 'APZR': 'ADP', 'ART': 'DET', 'CARD': 'NUM', 'FM': 'X', 'ITJ': 'X',
    'KOKOM': 'CONJ', 'KON': 'CONJ', 'KOUI': 'CONJ', 'KOUS': 'CONJ', 'NE': 'NOUN', 'NN': 'NOUN',
    'PDAT': 'PRON', 'PDS': 'PRON', 'PIDAT': 'PRON', 'PIAT': 'PRON', 'PIS': 'PRON', 'PPER': 'PRON',
...
UNIV_TAGS = {'.': 0, 'ADJ': 1, 'ADV': 2, 'ADP': 3, 'DET': 4, 'NUM': 5, 'CONJ': 6, 'NOUN': 7, 'PRON': 8, 'PRT': 9,
             'VERB': 10, 'X': 11}
...
def read_raw(file, keys, converters=None, raw=False):
    ...
    df = pd.read_csv(file, sep="\t", names=keys, header=None, usecols=usecols, dtype=dtype,
...
def get_preprocessed_corpus(corpus):
    ...
    return pd.read_csv(FILES[PREPROCESSED](corpus), sep="\t", dtype={TOKN_ID: int, SENT_ID: int}, na_filter=False)
...
def get_original_corpus(corpus, show_sample=0, raw=False):
    ...
    df = pd.concat((read_raw(file, KEYS[corpus], converters=CONVERTERS[corpus], raw=raw) for file in FILES[corpus]))
    df[CORP] = corpus
    if not raw:
        df = df[KEYS[DEFAULT]]
    if show_sample:
...
def get_selftagged_corpus(corpus=TIGER, framework=SPACY, show_sample=0):
    ...
    df = pd.read_csv(FILES[SELFTAGGED](corpus, framework), sep="\t", names=KEYS[SELFTAGGED],
...
def data_loader_main():
    """ this is loading the original corpora files, doing some preprocessing and saving them to disc.
    It would be too time consuming to do this preprocessing each time we want to access the data, so
    ...
    for corpus in process_corpora:
        ...
        print("{} done in {:f}s".format(corpus, time() - t0))
